...
def normalizing(c, col_name):
  if c[col_name] == 1:
    return 0
  else:
    return 1
  
df['cholesterol'] = df.apply(normalizing, col_name='cholesterol', axis=1)
df['gluc'] = df.apply(normalizing, col_name='gluc', axis=1)
  
#df.drop(df[df['ap_lo'] >= df['ap_hi']].index, inplace=True)
#print(df.iloc[225:250])
